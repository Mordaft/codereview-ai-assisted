...
function categoryLabel(category?: string) {
  ...
  if (category === 'security') return 'Seguridad'
  if (category === 'quality') return 'Calidad'
...
async function renderReviewWorkspace(review: Review, activeFileIndex = 0) {
  ...
  const workspaceComments = storedComments.length ? storedComments.map((comment) => ({
    ...
    author: comment.author ?? (comment.source === 'slm' ? 'SLM local' : comment.source === 'human' ? 'Revisor local' : 'Revisor'),
    ...
    message: comment.body,
    author: comment.author ?? 'Revisor remoto',
  ...
  const openProposalDialog = (context: {
    ...
    proposalAuthorInput.value = context.initialAuthor ?? 'Revisor local'
  ...
  document.querySelector('#new-file-proposal')?.addEventListener('click', () => openProposalDialog({ path: activeFile.path, allowGlobalToggle: true, initialAuthor: 'Revisor local', initialCategory: 'solid', initialDecision: 'pendiente' }))
  document.querySelectorAll<HTMLElement>('.code-line').forEach((lineElement) => lineElement.addEventListener('contextmenu', (event) => {
    ...
    openProposalDialog({ path: activeFile.path, line: lineNumber, allowGlobalToggle: false, initialAuthor: 'Revisor local', initialCategory: 'solid', initialDecision: 'pendiente' })
  }))
