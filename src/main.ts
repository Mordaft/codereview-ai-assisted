...
async function renderReviewWorkspace(review: Review, activeFileIndex = 0) {
  ...
  app.innerHTML = `<div class="review-workspace"><header class="workspace-header"><button class="back-button" id="back-to-reviews" type="button">← Revisiones</button><div class="workspace-title"><span class="provider provider--${review.provider.toLowerCase()}">${review.provider}</span><strong>${review.title}</strong><span class="workspace-repository">${review.repository}</span></div><div class="workspace-actions"><span class="status status--${statusClass(review.status)}"><span></span>${review.status}</span>${isReviewLocked ? '<button class="primary-action" data-decision="reopen" type="button">Reabrir</button>' : '<button class="outline-action" data-decision="close" type="button">Cerrar</button><button class="primary-action" data-decision="approve" type="button">Aprobar localmente</button>'}<button class="icon-button" id="workspace-theme-toggle" type="button" aria-label="Cambiar tema">◐</button></div></header><div class="workspace-grid"><aside class="file-tree"><div class="panel-label">FICHEROS MODIFICADOS <span>${workspaceFiles.length}</span></div><div class="tree-root">⌄ ${review.repository}</div>${workspaceFiles.map((file, index) => `<button class="file-item ${index === activeFileIndex ? 'file-item--active' : ''}" data-file-index="${index}" type="button"><span class="file-type">${file.type}</span><span>${file.path}</span>${workspaceComments.some((comment) => comment.file === file.path) ? '<b class="comment-count">2</b>' : ''}</button>`).join('')}<div class="tree-summary"><span class="live-dot"></span> ${workspaceComments.length} comentarios</div></aside><main class="code-review-panel"><div class="code-toolbar"><div><strong id="active-file-path">${activeFile.path}</strong><span>Vista de cambios · ${activeFile.lines.length} líneas</span></div><div class="code-toolbar__actions"><button class="outline-action" id="new-file-proposal" type="button">+ Propuesta</button></div></div><div class="code-frame">${activeFile.lines.map((line, index) => { const lineNumber = index + 1; const comments = activeFileComments.filter((comment) => comment.line === lineNumber); return `<div class="code-line ${comments.length ? 'code-line--commented' : ''}" data-line="${lineNumber}"><span class="line-number">${lineNumber}</span><code>${line.replaceAll('&', '&amp;').replaceAll('<', '&lt;')}</code>${comments.length ? `<span class="line-marker" title="${comments.length} comentario(s)">●</span>` : ''}</div>` }).join('')}</div></main><aside class="comments-panel"><div class="comments-header"><div><span class="panel-label">COMENTARIOS DE REVISION</span><h2>${activeFileComments.length} propuestas</h2><small class="comments-file">${activeFile.path}</small></div></div>${activeFileComments.length ? activeFileComments.map((comment) => `<article class="review-comment" data-comment-id="${comment.id ?? ''}"><div class="comment-meta"><span class="severity severity--${comment.severity}">${severityLabel(comment.severity)}</span><span>${comment.file ? `${comment.file}${comment.line ? ':' + comment.line : ''}` : 'Propuesta global'}</span></div><p>${comment.message}</p><small>${comment.source} · Pendiente de decisión</small>${comment.id && !isReviewLocked ? '<div class="comment-actions"><button class="comment-action comment-action--edit" type="button">Editar</button><button class="comment-action comment-action--delete" type="button">Eliminar</button></div>' : ''}</article>`).join('') : '<div class="comments-empty">Este fichero no tiene comentarios de revisión.</div>'}</aside></div></div>
    <dialog id="new-proposal-dialog" class="review-dialog bg-brand-surface-light text-brand-primary-light dark:!bg-brand-surface-dark dark:!text-brand-primary-dark"><form method="dialog" id="new-proposal-form"><button class="dialog-close" value="cancel" aria-label="Cerrar">×</button><span class="eyebrow">PROPUESTA</span><h2 id="new-proposal-title">Añadir propuesta</h2><p id="new-proposal-hint"></p><label for="proposal-message">Descripción</label><textarea id="proposal-message" name="proposal-message" rows="4" required placeholder="Describe la propuesta de revisión"></textarea><span class="field-label">Severidad</span><div class="severity-picker" id="proposal-severity-picker"><label class="severity-option severity-option--baja"><input type="radio" name="proposal-severity" value="baja"><span>Baja</span></label><label class="severity-option severity-option--media"><input type="radio" name="proposal-severity" value="media" checked><span>Media</span></label><label class="severity-option severity-option--alta"><input type="radio" name="proposal-severity" value="alta"><span>Alta</span></label></div><label class="proposal-global-toggle" id="proposal-global-toggle"><input type="checkbox" id="proposal-global" name="proposal-global"> No asociar a ningún fichero (propuesta global)</label><div class="dialog-actions"><button class="secondary-action" value="cancel">Cancelar</button><button class="primary-action" id="save-proposal" value="default">Guardar propuesta</button></div></form></dialog>
    <dialog id="delete-proposal-dialog" class="review-dialog bg-brand-surface-light text-brand-primary-light dark:!bg-brand-surface-dark dark:!text-brand-primary-dark"><form method="dialog" id="delete-proposal-form"><button class="dialog-close" value="cancel" aria-label="Cerrar">×</button><span class="eyebrow">ELIMINAR PROPUESTA</span><h2>¿Eliminar esta propuesta?</h2><p>Esta acción no se puede deshacer.</p><div class="dialog-actions"><button class="secondary-action" value="cancel">Cancelar</button><button class="primary-action primary-action--danger" value="default">Eliminar</button></div></form></dialog>
  `

  document.querySelector('#back-to-reviews')?.addEventListener('click', () => navigate('#/reviews'))
  document.querySelector('#workspace-theme-toggle')?.addEventListener('click', () => {
...
function renderSettings() {
  ...
  app.innerHTML = `<div class="settings-shell"><header class="settings-header"><button class="back-button" id="back-to-reviews" type="button">← Revisiones</button><div><span class="eyebrow">CONFIGURACION</span><h1>Runtime SLM local</h1></div><span class="settings-badge"><span class="live-dot"></span> Solo entorno local</span></header><main class="settings-content"><section class="settings-intro"><div class="settings-icon">✦</div><div><h2>Configura tu modelo de revisión</h2><p>Indica dónde está disponible el runtime SLM que analizará el código. La aplicación no enviará el contenido a servicios IA públicos.</p></div></section><form id="slm-config-form" class="settings-form" novalidate><div class="settings-section"><div><span class="panel-label">CONEXION</span><h2>Runtime compatible</h2><p>La URL debe apuntar a un servicio ejecutándose en este equipo.</p></div><div class="form-grid"><label>URL local del runtime<input id="slm-base-url" name="baseUrl" type="url" value="${config.baseUrl}" placeholder="http://localhost:11434/v1" required><small>Ejemplos: Ollama, llama.cpp o vLLM con API compatible.</small></label><label>Modelo SLM<div class="model-selector"><select id="slm-model" name="model" required><option value="${config.model}" selected>${config.model}</option></select><button class="refresh-models" id="refresh-models" type="button" aria-label="Actualizar modelos" title="Actualizar modelos">↻</button></div><small id="slm-model-hint">Cargando modelos disponibles...</small></label></div></div><div class="settings-section"><div><span class="panel-label">INFERENCIA</span><h2>Parámetros del análisis</h2><p>Se aplicarán a cada análisis inicial de la MR/PR.</p></div><div class="form-grid form-grid--compact"><label>Temperatura<input name="temperature" type="number" value="${config.temperature}" min="0" max="2" step="0.1" required><small>Valores bajos producen respuestas más deterministas.</small></label><label>Máximo de tokens<input name="maxTokens" type="number" value="${config.maxTokens}" min="256" max="32768" step="256" required><small>Límite de salida de cada análisis.</small></label></div></div><div class="settings-footer"><span id="slm-config-status" class="settings-status">La configuración se guarda en este dispositivo.</span><div><button class="outline-action" id="test-slm" type="button">Probar conexión</button><button class="primary-action" id="save-slm" type="submit">Guardar configuración</button></div></div></form></main></div>`
  ...
  settingsFooter.insertAdjacentHTML('beforebegin', `<div class="settings-section settings-section--prompts"><div><span class="panel-label">PROMPTS</span><h2>Contrato de revisión</h2><p>Personaliza las instrucciones sin alterar el formato que necesita la aplicación.</p></div><div class="prompt-fields"><label>Instrucciones de revisión<textarea id="review-instructions" name="reviewInstructions" rows="7" required>${promptConfig.reviewInstructions}</textarea><small>Estas instrucciones se enviarán al agente SLM junto con el código.</small></label><label>Contrato de salida bloqueado<textarea class="prompt-contract" rows="10" readonly aria-readonly="true">${reviewOutputContract}</textarea><small>El contrato es de solo lectura para garantizar que los comentarios puedan asociarse a fichero y línea.</small></label></div></div>`)
  document.querySelector('#back-to-reviews')?.addEventListener('click', () => navigate('#/reviews'))
...
function render(filter: 'active' | 'closed' = 'active') {
  ...
  const accessTokenField = hasRepositoryAccessToken()
    ? ''
    : '<label for="repository-token">Access token</label><input id="repository-token" name="repository-token" type="password" autocomplete="off" placeholder="Token de GitHub o GitLab" required><small class="token-hint">Se usara solo durante esta sesion y no se guardara en IndexedDB.</small>'
  app.innerHTML = `
    <div class="app-shell">
      <aside class="sidebar">
      ...
      <main class="content bg-brand-canvas-light text-brand-primary-light dark:!bg-brand-canvas-dark dark:!text-brand-primary-dark">
        ...
        <section class="intro bg-brand-secondary-light dark:!bg-brand-primary-light"><div><h2>Revisa con criterio, decide con claridad.</h2><p>Orquesta el analisis local y la revision humana en un unico espacio.</p></div><button class="primary-action" id="new-review" type="button"><span>+</span> Nueva revision</button></section>
        <section class="stats" aria-label="Resumen de revisiones"><div class="stat bg-brand-surface-light dark:!bg-brand-surface-dark"><span class="stat__label">En curso</span><strong>${reviewKpis.activeReviews}</strong><span class="stat__detail stat__detail--positive">Revisiones activas</span></div><div class="stat bg-brand-surface-light dark:!bg-brand-surface-dark"><span class="stat__label">Pendientes de IA</span><strong>${reviewKpis.pendingSlmComments}</strong><span class="stat__detail">Propuestas por revisar</span></div><div class="stat bg-brand-surface-light dark:!bg-brand-surface-dark"><span class="stat__label">Comentarios publicados</span><strong>${reviewKpis.publishedComments}</strong><span class="stat__detail stat__detail--positive">Total</span></div></section>
        <div class="section-heading"><div><h2>${filter === 'closed' ? 'Revisiones cerradas' : 'Revisiones activas'}</h2><p>${visibleReviews.length} procesos registrados</p></div><div class="tabs" role="tablist"><button class="tab ${filter === 'active' ? 'tab--active' : ''}" data-filter="active" type="button">Activas</button><button class="tab ${filter === 'closed' ? 'tab--active' : ''}" data-filter="closed" type="button">Cerradas</button></div></div>
        <section class="review-list">${visibleReviews.length ? visibleReviews.map(reviewCard).join('') : '<div class="empty-state"><strong>No hay revisiones cerradas</strong><span>Las revisiones finalizadas apareceran aqui.</span></div>'}</section>
        <footer class="content-footer"><span><span class="live-dot"></span> SLM local listo</span><span>Los datos permanecen en este dispositivo</span></footer>
      </main>
    </div>
    <dialog id="new-review-dialog" class="review-dialog bg-brand-surface-light text-brand-primary-light dark:!bg-brand-surface-dark dark:!text-brand-primary-dark"><form method="dialog" id="new-review-form"><button class="dialog-close" value="cancel" aria-label="Cerrar">×</button><span class="eyebrow">NUEVO PROCESO</span><h2>Conectar una revision</h2><p>Introduce la URL de un Pull Request o Merge Request para comenzar.</p><label for="review-url">URL de la MR / PR</label><input id="review-url" name="review-url" type="url" placeholder="https://github.com/empresa/repo/pull/42" required>${accessTokenField}<small id="remote-request-status" class="token-hint">Se consultaran los datos de la plataforma antes de crear la revision.</small><div class="dialog-actions"><button class="secondary-action" value="cancel">Cancelar</button><button class="primary-action" id="connect-review" value="default">Continuar</button></div></form></dialog>
  `
