const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./review-runtime-CWQ3Fhlx.js","./enums-5HL4wscD.js","./index-CLSXhyul.js","./rolldown-runtime-hePW80VL.js","./dexie-D3KjN6fK.js","./index-CbRxoG4b.css","./langgraph-CTtiDLzx.js","./review-prompts-BEeKHNk5.js"])))=>i.map(i=>d[i]);
import{a as e,i as t,l as n,n as r,o as i,r as a,s as o}from"./enums-5HL4wscD.js";import{A as s,C as c,D as l,H as u,I as ee,O as d,P as f,_ as p,a as m,b as h,d as te,g,h as _,l as ne,m as v,s as re,t as y,u as ie,v as b,w as x,y as S}from"./index-CLSXhyul.js";import{i as C}from"./platform-api-f3KosycC.js";function w(e){if(e==null)return i.PENDING;if(typeof e==`number`&&(e===i.DESIRABLE||e===i.IMPORTANT||e===i.BLOCKING||e===i.PENDING))return e;if(typeof e==`string`)switch(e.toLowerCase().trim()){case`1`:case`deseable`:case`desirable`:return i.DESIRABLE;case`2`:case`importante`:case`important`:return i.IMPORTANT;case`3`:case`bloqueante`:case`blocking`:return i.BLOCKING;default:return i.PENDING}return i.PENDING}async function ae(e){if(!e.id||!e.remoteChange?.webUrl)return;let n=(await d(e.id)).filter(e=>e.decision!==a.PUBLISHED);if(!n.length)return;let r=u(e.remoteChange.webUrl);for(let e of n){let n=e.path?`${e.path}${e.line?`:`+e.line:``}`:v(`workspace.generalComment`),i=`**[${b(e.severity??t.MEDIUM)}]** ${n}\n\n${e.body}`;await C(r,{body:i}),e.id&&await s(e.id)}}async function T(a,s){let{review:u,activeFileIndex:C=0,navigate:E,refreshReviews:D,onFileSelect:oe}=s,O=u.status===n.APPROVED||u.status===n.CLOSED||u.status===`Aprobada`||u.status===`Cerrada`,k=m(u),A=u.id?await d(u.id):[],j=u.remoteFiles?.length?u.remoteFiles.map(e=>({path:e.path,type:e.path.split(`.`).at(-1)?.toUpperCase()??`TXT`,lines:(e.content??e.patch??v(`workspace.noFilesDownloadedText`)).split(`
`)})):[{path:v(`workspace.noFilesDownloaded`),type:`TXT`,lines:[v(`workspace.noFilesDownloadedText`)]}],M=A.length?A.map(e=>({id:e.id,file:e.path,line:e.line,severity:e.severity??t.MEDIUM,message:e.body,author:_(e.author,e.source),category:e.category??r.SOLID,decision:w(e.decision),source:e.source})):u.remoteComments?.length?u.remoteComments.map(n=>({id:void 0,file:n.path,line:n.line,severity:t.MEDIUM,message:n.body,author:_(n.author,e.REMOTE),category:r.SOLID,decision:i.PENDING,source:e.REMOTE})):[],N=j[C]??j[0],P=M.filter(e=>!e.file||e.file===N.path),F=new Map;for(let e of M){if(!e.file)continue;let t=F.get(e.file)??[];t.push(e),F.set(e.file,t)}let I=u.id?c(u.id):void 0,se=I&&I.phase!==`completed`?`<span class="status status--thinking"><span class="thinking-pulse"></span>${I?.phase===`suggesting`?v(`reviews.card.generatingProposals`):v(`reviews.card.slmThinking`)}</span>`:`<span class="status status--${ie(u.status)}"><span></span>${S(u.status)}</span>`;a.innerHTML=`
    <div class="review-workspace bg-brand-canvas-light text-brand-primary-light dark:!bg-brand-canvas-dark dark:!text-brand-primary-dark">
      <header class="workspace-header bg-brand-surface-light border-brand-line-light dark:!bg-brand-surface-dark dark:!border-brand-line-dark">
        ...
        <div class="workspace-title">
          <span class="provider provider--${u.provider.toLowerCase()}">${u.provider}</span>
          <strong class="text-brand-primary-light dark:!text-brand-primary-dark">${u.title}</strong>
          <span class="workspace-repository text-brand-muted-light dark:!text-brand-muted-dark">${u.repository}</span>
        </div>
      ...
      <div class="workspace-grid">
        <aside class="file-tree bg-brand-surface-light border-brand-line-light dark:!bg-brand-surface-dark dark:!border-brand-line-dark">
          ...
          <div class="tree-root text-brand-muted-light dark:!text-brand-muted-dark">⌄ ${u.repository}</div>
          ${j.map((e,n)=>{let r=F.get(e.path)??[],i=r.length,a=r.some(e=>e.severity===t.HIGH||e.severity===`alta`)?`comment-badge comment-badge--alta`:`comment-badge`,o=v(`workspace.reviewProposalBadge`,{count:i,label:v(i===1?`workspace.proposalWordSingular`:`workspace.proposalWordPlural`)});return`<button class="file-item text-brand-muted-light dark:!text-brand-muted-dark ${n===C?`file-item--active dark:!bg-[#2e3e37] dark:!text-brand-primary-dark`:`hover:dark:!bg-[#263730]`}" data-file-index="${n}" type="button"><span class="file-type">${e.type}</span><span class="file-name" title="${e.path}">${e.path}</span>${i>0?`<span class="${a}" title="${o}" aria-label="${o}"><svg class="comment-badge__icon" width="10" height="10" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true"><path d="M2.5 2A1.5 1.5 0 0 0 1 3.5v7A1.5 1.5 0 0 0 2.5 12h2.5v2.793a.5.5 0 0 0 .854.353L8.707 12H13.5a1.5 1.5 0 0 0 1.5-1.5v-7A1.5 1.5 0 0 0 13.5 2h-11z"/></svg><span class="comment-badge__count">${i}</span></span>`:``}</button>`}).join(``)}
    ...
    <dialog id="new-proposal-dialog" class="review-dialog bg-brand-surface-light text-brand-primary-light dark:!bg-brand-surface-dark dark:!text-brand-primary-dark">
      <form method="dialog" id="new-proposal-form">
        <button class="dialog-close text-brand-muted-light dark:!text-brand-muted-dark" type="button" formnovalidate value="cancel" aria-label="${v(`common.close`)}">×</button>
        ...
        <div class="proposal-form-grid">
          ...
          <div class="proposal-field">
            ...
            <div class="severity-picker" id="proposal-severity-picker">
              <label class="severity-option severity-option--baja"><input type="radio" name="proposal-severity" value="${t.LOW}"><span>${b(t.LOW)}</span></label>
              <label class="severity-option severity-option--media"><input type="radio" name="proposal-severity" value="${t.MEDIUM}" checked><span>${b(t.MEDIUM)}</span></label>
              <label class="severity-option severity-option--alta"><input type="radio" name="proposal-severity" value="${t.HIGH}"><span>${b(t.HIGH)}</span></label>
            </div>
          </div>
        </div>
        ...
        <div class="dialog-actions">
          <button class="secondary-action bg-brand-surface-light border-brand-line-light text-brand-muted-light dark:!bg-brand-surface-dark dark:!border-brand-line-dark dark:!text-brand-muted-dark" type="button" formnovalidate value="cancel">${v(`common.cancel`)}</button>
          <button class="primary-action" id="save-proposal" type="submit" value="default">${v(`dialogs.proposal.saveProposal`)}</button>
        </div>
      </form>
    </dialog>
